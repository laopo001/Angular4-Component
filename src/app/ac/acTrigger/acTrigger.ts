...
export default class acTrigger {
  ...
  private getProjectableNodes(): { nodes: any[], viewRef?: EmbeddedViewRef<any> } {
    if (this.target instanceof TemplateRef) {
      const viewRef: EmbeddedViewRef<any> = this.viewContainer.createEmbeddedView(<TemplateRef<any>>this.target);
      return { nodes: viewRef.rootNodes, viewRef };
    } else {
      return { nodes: [this.renderer.createText(null, <string>this.target)] };
    }
  }
